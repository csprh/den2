...
def parse_args():
    ...
    parser.add_argument('--model_name', default='model_new_weights_065.hdf5', type=str, help='the model name')
...
if __name__ == '__main__':
    ...
    for set_cur in args.set_names:
        ...
        #        data1 = dataO.astype('float32')/255.0
                ...
                ssim_d0 = compare_ssim(np.squeeze(x), np.squeeze(d0))
                ...
                ssims_d0.append(ssim_d0)
                ssims_d1.append(ssim_d1)

        #if args.save_result:
        save_result(np.hstack((psnrs,ssims)),path=os.path.join(args.result_dir,set_cur,'results.txt'))
        save_result(np.hstack((psnrs_d0,ssims_d0)),path=os.path.join(args.result_dir,set_cur,'results_d0.txt'))
        save_result(np.hstack((psnrs_d1,ssims_d1)),path=os.path.join(args.result_dir,set_cur,'results_d1.txt'))
