...
def difCNN(depth,filters=64):
    ...
    for i in range(depth):
       ...
       x1 = Activation('relu')(x1)
       x1 = Lambda(lambda element: K.clip(element, 0, 1))(x1)
    # last layer, Conv
...
def findLastCheckpoint(save_dir):
    file_list = glob.glob(os.path.join(save_dir,'model_weights*.hdf5'))  # get name list of all .hdf5 files
...
if __name__ == '__main__':
    ...
    checkpointer = ModelCheckpoint(os.path.join(save_dir,'model_new_weights_{epoch:03d}.hdf5'),
                verbose=1, save_weights_only=False, period=args.save_every)
    csv_logger = CSVLogger(os.path.join(save_dir,'log_weights.csv'), append=True, separator=',')
